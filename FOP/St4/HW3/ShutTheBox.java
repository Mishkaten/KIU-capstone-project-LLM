public class ShutTheBox {
    ...
    public static void main(String[] args) {
        ...
        for (int round = 1; round <= 8; round++) {
            ...
            if (allBoxesClosed(boxes)) {
                MiniJava.printLine("Player 1 shuts all boxes! Player 1 wins!");
                return;
            }

            player2Score += playRound(2, boxes);
            if (allBoxesClosed(boxes)) {
                MiniJava.printLine("Player 2 shuts all boxes! Player 2 wins!");
                return;
            }
        }

        MiniJava.printLine("Player 1 points:\n<" + player1Score);
        MiniJava.printLine("Player 2 points:\n<" + player2Score);

        if (player1Score < player2Score) {
            MiniJava.printLine("Player 1 wins!");
        } else if (player2Score < player1Score) {
            MiniJava.printLine("Player 2 wins!");
        } else {
            MiniJava.printLine("Draw!");
        }
    }
    ...
    private static int playRound(int player, boolean[] boxes) {
        ...
        MiniJava.printLine("Player " + player + " has thrown the numbers:\n<" + dice1 + "\n<" + dice2);
        outputBoxes(boxes);
        ...
        while (true) {
            ...
            if (isValidChoice(boxes, box1, box2, sum)) {
                ...
                MiniJava.printLine("<Invalid choice. Try again.");
            }
        }
        return 0;
    }
    ...
    private static void outputBoxes(boolean[] boxes) {
        ...
        MiniJava.printLine(sb.toString());
    }
