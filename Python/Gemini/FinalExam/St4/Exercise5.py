...
def knapsack(items: list, max_weight: int) -> int:
    ...
    Args:
        items: A list of tuples, where each tuple represents an item (value, weight).
        max_weight: The maximum weight the knapsack can carry.

    Returns:
        The maximum value that can be achieved without exceeding the weight limit.
    """
    ...
    for i in range(1, num_items + 1):
        for w in range(1, max_weight + 1):
            ...
            if weight > w:
                dp[i][w] = dp[i - 1][w]  # Item is too heavy, don't take it
            else:
                dp[i][w] = max(
                    dp[i - 1][w],  # Don't take the item
                    dp[i - 1][w - weight] + value  # Take the item
                )

    # Optimal Value Calculation (1 point)
    return dp[num_items][max_weight]  # The bottom-right cell holds the max value
